...
def f(id, moji, cd=0):
    ...
    a = requests.get(product_url, headers=headers, cookies=None)
    ...
    if "ワンクリックで買う" in a.text:
        lines=a.text.splitlines()
        txt=""
        for line in lines:
            if "<title>" in line:
                txt=line.replace("<title>","").replace("</title>","")
                break
        print("detectd: " + moji+" : "+str(txt)+" : "+product_url)
    else:
        return id
    noti = requests.post("https://maker.ifttt.com/trigger/pc4u/with/key/dN390pdhTwKRlHT4443oOa", json={ "id":"Udc7d580e086b4e8b7f956638fcb316aa", "value1": "dos bot detected:%s<br>%s" % (moji, str(txt)), "value2": product_url})
    ...
    if "申し訳ございません" not in a.text and "申し訳ありません" not in a.text:
        ...
        print(id)
        print(a.text)
        csno = a.cookies.get("csno")
        csid = a.cookies.get("csid")
        print(csno)
        print(csid)
        noti = requests.post("https://maker.ifttt.com/trigger/pc4u/with/key/dN390pdhTwKRlHT4443oOa", json={ "id":"Udc7d580e086b4e8b7f956638fcb316aa", "value1": "dos bot cartin: %s %s<br>%s<br>%s" % (moji, str(txt), csid, csno), "value2": product_url})
        # print(a.cookies.items)
        time.sleep(5)
    elif "データメンテナンス" in a.text:
        print(a.text)
        print(id)
        noti = requests.post("https://maker.ifttt.com/trigger/pc4u/with/key/dN390pdhTwKRlHT4443oOa", json={ "id":"Udc7d580e086b4e8b7f956638fcb316aa", "value1": "dos bot: mentainance %s %s" % (moji, str(txt)), "value2": product_url})
    else:
        # print(a.text)
...
gpu3 = ['469657', '469318', '469208', '469207', '469205', '469206', '471205', #'470802'
        '469319', '468874', '469335', '469310', '470790', '469336', '469309', '468617', '473592'] #'468873',
gpu4 = ['468868', '468628', '468631', '468627', '468620', '469631', '469126', '470310', '470388', '471596',
        '468430', '469630', '468621', '469147', '468611', '468649', '472481', '468614', '468429', '469925']
f0 = False
f1 = f2 = f3 = f4 = True
...
r3 = []
r4 = []
exec = concurrent.futures.ThreadPoolExecutor(max_workers=5)
match = "search list:"
if f0:
    match = match+" HDD"
    for r in gpu:
